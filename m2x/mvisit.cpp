...
short trovaConfigurazione(const char *sglaut, const char *sglope)
{
  ...
  while (iConf < nConf)
  {
    ...
    if (strcmp(c->sglaut, sglaut) == 0 && strcmp(c->sglope, sglope) == 0)
      break;
...
void visitRiga(short nRow,
               ...
               struct testo *sigle)
{
  if (riga->key != HLINEKEY)
  {
    printRowHeader(riga->key, parametri);
    int nCampi = visitCampi(nRow, colonne, riga, parametri, sigle);
    if (nCampi > 2)
    {
                ...
                struct testo *sigle)
{
  ...
  printTableFinish(key, tabella, parametri, sigle);
}
...
bool unicTestRif(struct testo *sigle, char *testRif)
{
  ...
  if (testRif)
  { // SE IL TESTIMONE DI RIFERIMENTO E` STATO IMPOSTATO

    if (sigle)
    { // SE CI SONO ALTRE SIGLE NELLA LISTA
...
// CERCA LA VARIANTE DI RIFERIMENTO RISPETTO A QUELLA CHE SI RIFERISCE AL TESTIMONE RICHIESTO
struct testo *findVariantePrecedente(struct testo *testo, char *testimone)
{
  struct testo *variante = NULL;
  if (testo->testo2 != NULL && testimone != NULL)
  { // CERCO LA SIGLA DEL TESTIMONE RICHIESTO NELLA VARIANTE SUCCESSIVA
    if (findSigle(testo->testo2->testo1, testimone))
    {
      variante = testo;
    }
    else
    { // CERCO LA SIGLA DEL TESTIMONE RICHIESTO NELLE VARIANTI SUCCESSIVE
      variante = findVariantePrecedente(testo->testo2, testimone);
    }
  }
  return variante;
}
...
{
  const char *label = NULL;
  struct testo *variante;
  struct testo *lezione;
  struct testo *variantePrec;
  variantePrec = findVariantePrecedente(testo, opzioneTestEx());
  if (variantePrec != NULL)
  {
    variante = variantePrec->testo2->testo1->testo2;
    lezione = variante->testo1;
    if (lezione == NULL)
    {
      if (opzioneVariantePrecedente())
      { // LEZIONE VARIANTE PRECEDENTE
        lezione = variantePrec->testo1->testo2->testo1;
      }
      else
      { // LEZIONE PRIMA VARIANTE
        lezione = testo->testo1->testo2->testo1;
      }
    }
  }
  else
  { // TESTIMONE NON TROVATO: CONSIDERA LA PRIMA VARIANTE
...
void visitVF(struct testo *testo, PARAMETRI *parametri, struct testo *sigle)
{
  if (checkVarianti(testo, opzioneTestEx()))
  {
    printVF(testo, parametri, sigle);
  }
}
...
short visitVarianti(
    ...
    bool multiplex,
    int numero,
    struct testo *lezioneRiferimento)
{
  short nTest = 0;
  // SE CI SONO ALTRE VARIANTI NELLA LISTA
  if (testo)
  { // STAMPA LA PRIMA VARIANTE ED UNO SPAZIO SE QUESTA NON E` VUOTA
    struct testo *lezione = testo->testo1->testo2->testo1;
    if (lezione == NULL)
    {
      lezione = lezioneRiferimento;
    }
    struct testo *lezioneRif = opzioneVariantePrecedente() || lezioneRiferimento == NULL ? lezione : lezioneRiferimento;
    ...
    if (testo->lexema)
    {
      if (strcmp(testo->lexema, "duplex") == 0)
        duplex = true;
    }
    // VERIFICA SE SI TRATTA DI UNA TRASPOSIZIONE "MULTIPLEX"
    if (testo->lexema)
    {
      if (strcmp(testo->lexema, "multiplex") == 0)
        multiplex = true;
    }
    // STAMPA LE ALTRE VARIANTI
    nTest += visitVarianti(testo->testo2, parametri, sigle, false, duplex, multiplex, numero + 1, lezioneRif);
  }
    ...
    short *nTest)
{
  ...
  if (testo && (opzioneBanale() || testo->key != BANALEKEY))
  {
              ...
              struct testo *sigle)
{
  ...
  case OMFRACKEY:
  case MFRACOKEY:
    printMFrac(key, testo, parametri, sigle);
    break;
  case PHANTOMKEY:
                ...
                struct testo *sigle)
{
  ...
  while (testo != NULL)
  {
    ...
    case ADNOTKEY:
      ...
      if (!parametri->nota)
      {
        printAdnotatio(testo->lexema, testo->testo1, parametri, sigle);
      }
      break;
    case FOOTNOTEKEY:
      parametri->nFootnote++;
      ...
      break;
    case VFKEY:
      visitVF(testo->testo1, parametri, sigle);
      break;
    case VBKEY:
    case SCHOLKEY:
    case VVKEY:
    ...
    case NMARGKEY:
    case NOTEKEY:
      if ((testo->key != VBKEY || opzioneBanale()) && !parametri->nota && !parametri->banale && !parametri->cit)
      {
        parametri->nVV++;
      }
