#define VERSIONSTRING "versione 1.14b9 del 17 ottobre 2024"
...
/*
  ...
  eLabor sc
  via Enrico Fermi 19, 56126 Pisa - Italia
  <http://elabor.biz>
  <info@elabor.biz>
...
void printTesto(struct testo *testo, const char *indent)
{
  if (testo != NULL)
  {
    printf("%s - %ld - %s)\n", indent, testo->key, testo->lexema);
    int len = strlen(indent);
    char newIndent[len + 2];
    strcpy(newIndent, indent);
    strcat(newIndent, ".1");
    printTesto(testo->testo1, newIndent);
    newIndent[len + 1] = '2';
    printTesto(testo->testo2, newIndent);
  }
}

void printHTMLProlog(FILE *file)
{
  if (singleMode)
  {
    fprintf(file, "<script src=\"https://polyfill.io/v3/polyfill.min.js?features=es6\"></script>\n");
    fprintf(file, "<script id=\"MathJax-script\" async=\"true\" src=\"https://cdn.jsdelivr.net/npm/mathjax@3.0.1/es5/tex-mml-chtml.js\"></script>\n");
  }
}
...
const char *getMacro(const char *lexema)
{
  ...
  if (strcmp(lexema, "\\c e") == 0)
    macro = "<strike>e</strike>";
  if (strcmp(lexema, "\\c p") == 0)
    macro = "<strike>p</strike>";
  ...
  if (strcmp(lexema, "\\it\\&") == 0)
    macro = "<em><strong>&amp;</strong></em>";
  if (strcmp(lexema, "\\.o") == 0)
    macro = "<strike>o</strike>";
...
void printHidingSubnote(struct testo *testo, PARAMETRIm2h *parametri, struct testo *sigle)
{
  if (testo != NULL)
  {
    if (testo->key == VVKEY)
    {
      // fprintf(parametri->outFile, " %ld ", testo->key);
      fprintf(parametri->outFile, " (");
      visitVarianti(testo->testo1, parametri, sigle, true, false, false, 1, NULL);
      fprintf(parametri->outFile, ")");
    }
    printHidingSubnote(testo->testo1, parametri, sigle);
    printHidingSubnote(testo->testo2, parametri, sigle);
  }
}
...
void printTableStart(int key, struct testo *testo, PARAMETRI *param, struct testo *sigle)
{
  ...
  int nVariante = parametri->nVariante;
  if (parametri->nota)
  {
    if (nVariante == 1 && (sigle == NULL || sigle->lexema == NULL || strcmp(sigle->lexema, "*") == 0 || strcmp(sigle->lexema, "+") == 0))
    {
      fprintf(parametri->outFile,
              "<table style=\"display: inline-block; border: 2px solid black; padding: 5px; background: lightgrey;\">\n");
      parametri->hide = true;
      fprintf(parametri->outFile, "<tbody style=\"display: none;\">\n");
      fprintf(parametri->outFile, "<tr><td>\n\n");
    }
    else
    {
      if (!parametri->hide)
      {
        fprintf(
            parametri->outFile,
            "<img style=\"border: 0; vertical-align: middle;\" src=\"./figure.html?fig=schema-apparato-%d.svg\"></img>",
            nSvgA);
        parametri->hide = true;
        fprintf(parametri->outFile, "<table style=\"display: none\"><tbody><tr><td>\n\n");
        nSvgA++;
      }
    }
  }
  else
  {
    ...
    sprintf(filePath, "./figure/schema-%d.svg", nSvg);
    if (access(filePath, F_OK) == 0)
    {
      char const *position = parametri->centering ? "" : " float: inline-end;";
      fprintf(
          parametri->outFile,
          "<img style=\"border: 0; vertical-align: middle;%s margin:5px;\" src=\"./figure.html?fig=schema-%d.svg\"></img>", position, nSvg);
      fprintf(parametri->outFile, "<table style=\"display: none\"><tbody><tr><td>\n\n");
    }
    else
    {
      fprintf(parametri->outFile,
              "<table style=\"display: inline-block; border: 0; vertical-align: middle;\" cellpadding=\"0\" cellspacing=\"4\"><tbody><tr><td>\n\n");
    }
    nSvg++;
  }
...
void printTableFinish(int key, struct testo *tabella, PARAMETRI *param, struct testo *sigle)
{
  PARAMETRIm2h *parametri = (PARAMETRIm2h *)param;
  struct testo *testo = tabella->testo1->testo2;
  fprintf(parametri->outFile, "</td><td>\n\n");
  visitTesto(testo, parametri, sigle);
  fprintf(parametri->outFile, "</td></tr></tbody></table>\n\n");
  parametri->hide = false;
  if (parametri->nota)
  {
    printHidingSubnote(tabella, parametri, sigle);
  }
}
...
void printTableHeader(int key, const char *colonne, PARAMETRI *parametri)
{
  ...
  {
    fprintf(parametri->outFile, "<table style=\"border: 1px solid black\">\n");
  }
}
...
void printPhantom(struct testo *testo, PARAMETRI *parametri, struct testo *sigle)
{
  ...
  fprintf(parametri->outFile, "\\phantom{");
...
void printFrac(struct testo *testo, PARAMETRI *parametri, struct testo *sigle)
{
  fprintf(parametri->outFile, "{");
  visitTesto(testo->testo1, parametri, sigle);
  fprintf(parametri->outFile, " \\over ");
  visitTesto(testo->testo2, parametri, sigle);
  fprintf(parametri->outFile, "}");
}

void printParte(struct testo *testo, PARAMETRI *parametri, struct testo *sigle)
{
  while (testo)
  {
    if (testo->lexema && strcmp(testo->lexema, ",") == 0)
    {
      fprintf(parametri->outFile, "~~");
    }
    else
    {
      if (testo->key == PHANTOMKEY)
      {
        printPhantom(testo, parametri, sigle);
      }
      else
      {
        printLexema(testo->key, testo->lexema, parametri);
...
void printMFracPart(struct testo *testo, PARAMETRI *parametri, struct testo *sigle)
{
  while (testo)
  {
    printParte(testo->testo1, parametri, sigle);
    testo = testo->testo2;
  }
}
...
void printMFrac(short key, struct testo *testo, PARAMETRI *parametri, struct testo *sigle)
{
  ...
  fprintf(parametri->outFile, "%s{", prefix);
  printMFracPart(testo->testo1, parametri, sigle);
  fprintf(parametri->outFile, " \\over ");
  printMFracPart(testo->testo2, parametri, sigle);
  fprintf(parametri->outFile, "}%s", suffix);
}
...
{
  // ENCLOSED ALPHANUMERICS (da 0 a 20)
  if (strcmp(lexema, "0") == 0)
  {
    fprintf(outFile, "&#x24EA;");
  }
  else if (strcmp(lexema, "1") == 0)
  {
    fprintf(outFile, "&#9312;");
  }
  ...
  {
    fprintf(outFile, "<span style=\"font-size:small;top:-0.2em;position:relative;\">&#9675;</span>");
  }
}
                ...
                bool multiplex)
{
  if (!duplex && !multiplex)
  {
    printNota(" &#126; ", parametri->outFile); // "TILDE"
  }
  rif *l = findLabel(label);
  if (l)
  {
    // SE L'ETICHETTA ESISTE
    short labelNUnit = l->nUnit;
    const char *label = findUnitLabel(l->nUnit);
    if (label != NULL && numeric(label))
    {
      labelNUnit = atoi(label);
    }
    if (parametri->nUnit < labelNUnit)
    {
      printUnitRef(labelNUnit, parametri);
      fprintf(parametri->outFile, " ");
    }
    // SALVA I PARAMETRI
  ...
  */
  ...
  printf(parametri->outFile, "foliumInTesto\n");
    ...
    case UNITKEY:
      if (inizio)
      {
        if (testo != NULL && testo->lexema != NULL && numeric(testo->lexema))
        {
          parametri->nUnit = atoi(testo->lexema);
        }
    ...
    case REFKEY:
      if (inizio)
      {
        ...
        fprintf(parametri->outFile,
                "<a href=\"./paragrafo.html?intro=%s&section=%s\">startreference/%s/%s/endreference</a>",
                label, ref, label, ref);
      }
      break;
    case ':':
    ...
    case CENTERKEY:
      if (inizio)
      {
        ...
        parametri->centering = true;
      }
      else
      {
        fprintf(parametri->outFile, "</center>");
        parametri->centering = false;
      }
      break;
    case PARAGRAPH:
      if (inizio)
...
void printFig(short key, struct testo *testo, PARAMETRI *param)
{
  ...
  else
  {
    ...
    if (parametri->nota)
    {
      if (parametri->nVariante == 1)
      {
        fprintf(parametri->outFile,
                "<table style=\"display: inline-block; border: 2px solid black; padding: 5px; background: lightgrey;\">\n");
        fprintf(parametri->outFile, "<tbody style=\"display: none;\">\n");
        fprintf(parametri->outFile, "<tr><td></td></tr>\n");
        fprintf(parametri->outFile, "</tbody></table>\n");
      }
      else
      {
        sprintf(filePath, "./figure/schema-apparato-%d.svg", nSvgA);
        if (access(filePath, F_OK) == 0)
        {
          fprintf(
              parametri->outFile,
              "<img style=\"border: 0; vertical-align: middle;\" src=\"./figure.html?fig=schema-apparato-%d.svg\"></img>", nSvgA);
          nSvgA++;
        }
        else
        {
          fprintf(
              ...
              fileName, parametri->nFig, figExtension, fileName, parametri->nFig, figExtension, parametri->nFig);
        }
      }
    }
    else
                   ...
                   struct testo *sigle)
{
  ...
  fprintf(parametri->outFile, "<a name=\"FOOT%d\"></a>", parametri->nFootnote);
    ...
    struct testo *sigle)
{
  ...
  fprintf(parametri->outFile, "<a name=\"ADN%d\"></a>", parametri->nAdnot);
             ...
             struct testo *sigle)
{
  ...
  else if (!parametri->cit)
  {
    ...
    if (vvFile)
    {
      ...
      {
        visitVarianti(testo, parametri, sigle, false, false, false, 1, NULL);
      }
      fprintf(vvFile, "</li>\n\n");
      // CHIUSURA DEL FILE DELLE NOTE
      fclose(vvFile);
...
int main(int argn, char **argv)
{
  ...
  else
  {
    ...
    if (help)
    {
      ...
      printf("  -s                   genera dei file che possono essere visualizzati singolarmente\n");
    }
  }
