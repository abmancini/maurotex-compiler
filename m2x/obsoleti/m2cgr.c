#define VERSIONSTRING "versione 0.4 luglio 2020"
...
#define MODEST	 "fmod.htm" //cgr-mod
#define FREST	 "r.htm" //cgr-mod
...
static char * testD1;          // PRIMO TESTIMONE PER CONCORDANZE
static char * testD2;          // SECONDO TESTIMONE PER CONCORDANZE
static bool   rcheck;		   // cgr-mod
static char * htmlFileName;
static char * vvFileName;
static char * rmFileName;	   // cgr-mod
static char * indFileName;
static char * erFileName[MAXTEST];
static char * coFileName;
// static char * rFileName;	   // cgr-mod - eliminato da Paolo 10 aprile 2021: sembra essere uno sbaglio
...
short printCorpoVariante(struct testo * lezione,
  ...
  if (nVar != 1) {
    ...
    // TEST ERRORI CONTRO CONCORDANZE
    if (findTestRif(sigle, testD1)) {
      singleErr = true;
    }
    if (findTestRif(sigle, testD2)) {
      findErr = true;
    }

    // VERIFICA PRESENZA CONCORDANZE IN ERRORE
    if (findTestRif(sigle, testD1) && findTestRif(sigle, testD2)) {
	...
	fprintf(coFile, "<a href=\"%s#VV%d\" target=\"variante\">%d</a>\n",
		vvFileName, parametri->nVV, parametri->nVV);
	...
	fprintf(stderr, "impossibile aprire il file %s\n", coFileName);
      ...
      // CHECK ERRORI CONTRO CONCORDANZE
      concErr = true;
    }
...
// FUNZIONE DI STAMPA DELL'EPILOGO HTML - cgr-mod
void printFineMod(FILE * htmlFile) {
	//fprintf(htmlFile, "\n</p>");
	fprintf(htmlFile, "\n\n</body>");
	fprintf(htmlFile, "\n</html>\n");
}
...
void nuovaPagina(char         * fileName,
  ...
  if (parametri->nPag == 1) {
    ...
    frameFileNameMod = new char[strlen(fileName)+strlen(MODEST)+10]; //cgr-mod
    rmFileName 	  = new char[strlen(fileName)+strlen(FREST)+10]; //cgr-mod
  }
  ...
  if(rcheck){
    sprintf(frameFileNameMod, "%s-%d-%s-%s-%s", fileName, parametri->nPag,testD1, testD2, MODEST); //cgr-mod
    sprintf(rmFileName, "%s-%d-%s-%s-%s", fileName, parametri->nPag, testD1, testD2, FREST); //cgr-mod
  }
  ...
  if (!livello) {
    ...
    if (testD1 != NULL && testD2 != NULL) {
      ...
      coFileName = new char[strlen(fileName)+strlen(COEST)+strlen(testD1)+strlen(testD2)+4];
      sprintf(coFileName, "%s-%s-%s-%s", fileName, testD1, testD2, COEST);
      ...
      if (coFile) {
	printPrologo(coFile);
	fprintf(coFile, "<h2>Concordanze di %s e %s vs. testo</h2>\n", testD1, testD2);
	fclose(coFile);
      } else {
	fprintf(stderr, "impossibile aprire il file %s\n", coFileName);
      }
    ...
    // FILE MODR - cgr-mod
    if (testD1 != NULL && testD2 != NULL && rcheck) {
      
      // GENERAZIONE DEL FILE
      FILE * rFile = fopen(rmFileName, "w");
      if (rFile) {
	printPrologo(rFile);						
	fprintf(rFile, "<h2> Testo - %s vs. %s</h2>\n", testD1, testD2);
	fclose(rFile);
      } else {
	fprintf(stderr, "impossibile aprire il file %s\n", rmFileName);
      }
    ...
    //cgr-mod --- frame-start
    // GENERAZIONE DEL FRAMESETMOD
    FILE * frameFileMod = fopen(frameFileNameMod, "w");
    if (frameFileMod) {
      
      // FRAMESET PRINCIPALE
      fprintf(frameFileMod, "<FRAMESET COLS=\"10%%, 55%%, 35%%\">\n");
      
      // FRAME DELL INDICE
      fprintf(frameFileMod, "<FRAME SRC=\"%s\" NAME=\"ind\">\n", indFileName);
      
      fprintf(frameFileMod, "<FRAMESET ROWS=\"80%%, 20%%\">\n");
      fprintf(frameFileMod, "<FRAME SRC=\"%s\" NAME=\"tc\">\n", htmlFileName);
      fprintf(frameFileMod, "<FRAME SRC=\"%s\" NAME=\"adnotatio\">\n", rmFileName);
      fprintf(frameFileMod, "</FRAMESET>");
      
      // FRAMESET DI VARIANTI E CITAZIONI
      fprintf(frameFileMod, "<FRAMESET ROWS=\"50%%, 50%%\">\n");
      fprintf(frameFileMod, "<FRAME SRC=\"%s\" NAME=\"variante\">\n", vvFileName);
      fprintf(frameFileMod, "<FRAME SRC=\"%s\" NAME=\"cit\">\n", citFileName);
      fprintf(frameFileMod, "</FRAMESET>\n");
...
void printFineVarianti(PARAMETRI * parametri) {
  ...
  // ERRORI CONTRO CONCORDANZE
  if (findErr && (!singleErr) && (!concErr)) {
      FILE * rFile = fopen(rmFileName, "a");
      if (rFile) {
	fprintf(rFile, "<a href=\"%s#VV%d\" target=\"variante\">%d</a>\n",
		vvFileName, parametri->nVV, parametri->nVV);
	fclose(rFile);
      } else {
	fprintf(stderr, "impossibile aprire il file %s\n", rmFileName);
...
int main (int argn, char ** argv) {
	...
	if (ok && !help) {
		...
		printf("uso: %s [-a] [-b] [-h] [-l0/1/2] [-n] [-p] [-v] [-x] [-y]\n", argv[0]);
		printf("\t[-f <file>]\n");
		printf("\t[-s <testimone>] [-S <testimone>]\n");
		printf("\t[-o <testimone>] [-O <testimone>]\n");
		printf("\t[-e <testimone>] [-E <testimone>]\n");
		printf("\t[-c <test_1> <test_2>] [-C <test_1> <test_2>]\n");
		printf("\t[-t <testimone>]\n");
		printf("\t[-r <test_1> <test_2>] [-R <test_1> <test_2>]\n");
		printf("\t<nome_file>\n");
		...
		if (help) {
			...
			printf("  -c <test_1> <test_2> genera il file concordanze tra <test_1> e <test_2>\n");
			...
			printf("  -r <test_1> <test_2> genera i file per lo studio di TestoBase-Testimone1 vs Testimone2\n");
